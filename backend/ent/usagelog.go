...
type UsageLog struct {
	...
	// EstimatedContextTokens holds the value of the "estimated_context_tokens" field.
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldEstimatedContextTokens:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field estimated_context_tokens", values[i])
			} else if value.Valid {
				_m.EstimatedContextTokens = new(int)
				*_m.EstimatedContextTokens = int(value.Int64)
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.EstimatedContextTokens; v != nil {
		builder.WriteString("estimated_context_tokens=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
