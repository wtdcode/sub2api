...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		estimatedContextTokens    sql.NullInt64
	...
	if estimatedContextTokens.Valid {
		value := int(estimatedContextTokens.Int64)
		log.EstimatedContextTokens = &value
	}
	if userAgent.Valid {
