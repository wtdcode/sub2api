...
// ===== OpenAI 上下文窗口声明（openai 平台的 apikey/upstream 账号） =====

export const CONTEXT_LENGTH_CREDENTIAL_KEY = 'context_length'

/** 上下文窗口声明资格（与后端读取 credentials.context_length 的范围一致） */
export function isContextLengthCapable(platform: string, type: string): boolean {
  return platform === 'openai' && (type === 'apikey' || type === 'upstream')
}

/**
 * 从凭据读取 context_length，仅接受正整数；0/缺失/脏数据视为未限制，返回 null
 * （与后端 0/absent = unlimited 语义对齐）。
 */
export function readContextLength(
  credentials: Record<string, unknown> | undefined | null
): number | null {
  const v = credentials?.[CONTEXT_LENGTH_CREDENTIAL_KEY]
  return typeof v === 'number' && Number.isInteger(v) && v > 0 ? v : null
}

/**
 * 把上下文窗口声明写入凭据：正整数则设置，其余（空/0/负数/非整数/NaN）删除该键
...
export function applyContextLength(
  ...
  if (typeof value === 'number' && Number.isInteger(value) && value > 0) {
    credentials[CONTEXT_LENGTH_CREDENTIAL_KEY] = value
  } else {
    delete credentials[CONTEXT_LENGTH_CREDENTIAL_KEY]
  }
}
